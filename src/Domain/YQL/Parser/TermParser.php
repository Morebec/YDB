...
class TermParser
{
    ...
    private function processFromAt(int $index, array $tokens): array
    {
        ...
        $ret = $this->processTableAt($index + 1, $tokens);
        $ret[] = $tokens[$index];
        return $ret;
    }
     ...
     * @return Token[]
     */
    private function processTableAt(int $index, array $tokens): array
    {
    ...
    private function processWhereAt(int $index, array $tokens): array
    {
        ...
        if($token->getType()->isEqualTo(TokenType::IDENTIFIER())) {
            $this->processTermAt($index + 1, $tokens);
            $ret[] = $tokens[$index];
            return $ret;
        }
    ...
    private function processTermAt(int $index, array $tokens): array
    {
        ...
        $this->expectOneInTokenTypeAt([TokenType::IDENTIFIER()], $columnIndex, $tokens);
        ...
        $column = $tokens[$columnIndex];
        $operator = $tokens[$operatorIndex];
        $value = $tokens[$valueIndex];

        $term = new TermToken($column, $operator, $value);
